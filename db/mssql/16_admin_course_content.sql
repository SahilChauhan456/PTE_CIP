...
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('training_courses') AND name = 'short_description')
  ALTER TABLE training_courses ADD short_description NVARCHAR(MAX);

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('training_courses') AND name = 'cover_image_url')
  ALTER TABLE training_courses ADD cover_image_url NVARCHAR(MAX);

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('training_courses') AND name = 'thumbnail_path')
  ALTER TABLE training_courses ADD thumbnail_path NVARCHAR(MAX);

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('training_courses') AND name = 'category')
  ALTER TABLE training_courses ADD category NVARCHAR(255);

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('training_courses') AND name = 'instructor_name')
  ALTER TABLE training_courses ADD instructor_name NVARCHAR(255);

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('training_courses') AND name = 'is_admin_created')
  ALTER TABLE training_courses ADD is_admin_created BIT NOT NULL DEFAULT 0;
GO
...
BEGIN
  ...
  CREATE INDEX idx_course_content_items_course ON course_content_items(course_id);
  CREATE INDEX idx_course_content_items_order ON course_content_items(course_id, display_order);
END
GO
...
BEGIN
  CREATE TABLE dbo.content_item_progress (
    enrollment_id UNIQUEIDENTIFIER NOT NULL REFERENCES training_enrollments(id) ON DELETE CASCADE,
    content_item_id UNIQUEIDENTIFIER NOT NULL REFERENCES course_content_items(id) ON DELETE CASCADE,
    completed_at DATETIMEOFFSET NOT NULL DEFAULT SYSDATETIMEOFFSET(),
    PRIMARY KEY (enrollment_id, content_item_id)
  );

  CREATE INDEX idx_content_item_progress_enrollment ON content_item_progress(enrollment_id);
END
GO
...
BEGIN
  ...
  DECLARE @done INT;
  DECLARE @pct INT;
  DECLARE @cur_status NVARCHAR(50);
  ...
  SELECT @cur_status = status FROM training_enrollments WHERE id = @enrollment_id;
  ...
  UPDATE training_enrollments
     SET progress_percent = @pct,
         status = @new_status,
         completed_at = CASE 
           WHEN @new_status = 'Completed' THEN COALESCE(completed_at, SYSDATETIMEOFFSET())
           ELSE NULL 
         END
