  ...
  SELECT tc.id, tc.course_code, tc.title, tc.short_description, tc.description,
         ...
         COALESCE((
           SELECT cci.id                AS id,
                  ...
                  cci.video_file_path    AS video_file_path,
                  cci.image_url          AS image_url,
                  cci.image_file_path    AS image_file_path,
                  cci.pdf_url            AS pdf_url,
                  cci.pdf_file_path      AS pdf_file_path,
                  cci.text_content       AS text_content,
                  cci.external_url       AS external_url,
                  cci.duration_minutes   AS duration_minutes,
                  (SELECT TOP 1 ecp.completed_at FROM employee_content_progress ecp WHERE ecp.content_item_id = cci.id AND ecp.employee_id = $3) AS completed_at
           FROM course_content_items cci
           WHERE cci.course_id = tc.id
           ORDER BY cci.display_order
      ...
      SELECT tc.id, tc.course_code, tc.title, tc.short_description, tc.description,
             ...
             COALESCE((
               SELECT cci.id                AS id,
                      ...
                      cci.external_url       AS external_url,
                      cci.duration_minutes   AS duration_minutes,
                      (SELECT TOP 1 ecp.completed_at FROM employee_content_progress ecp WHERE ecp.content_item_id = cci.id AND ecp.employee_id = $4) AS completed_at
               FROM course_content_items cci
               WHERE cci.course_id = tc.id
               ORDER BY cci.display_order
               FOR JSON PATH, INCLUDE_NULL_VALUES
...
async function getCourseProgress(employeeId, courseId) {
  const { rows } = await query(sql({
    ...
    mssql: `SELECT count(*) AS total,
                   count(ecp.content_item_id) AS done,
                   COALESCE(tc.duration_hours, 0) AS duration_hours
              FROM course_content_items cci
              JOIN training_courses tc ON tc.id = cci.course_id
              LEFT JOIN employee_content_progress ecp
...
async function syncPlanItem(employeeId, courseId, progress) {
  ...
  if (existing.rows.length) {
        await query(sql({
       pg: `UPDATE learning_plan_items SET status = $1, progress_percent = $2,
         completed_at = CASE WHEN $1 = 'Completed' THEN COALESCE(completed_at, NOW()) ELSE NULL END
         WHERE employee_id = $3 AND course_id = $4`,
       mssql: `UPDATE learning_plan_items SET status = $1, progress_percent = $2,
         completed_at = CASE WHEN $1 = 'Completed' THEN COALESCE(completed_at, SYSDATETIMEOFFSET()) ELSE NULL END
         WHERE employee_id = $3 AND course_id = $4`,
        }),
    ...
    await query(sql({
      pg: `INSERT INTO learning_plan_items (employee_id, course_id, status, progress_percent, completed_at)
          VALUES ($1, $2, $3, $4, CASE WHEN $3 = 'Completed' THEN NOW() ELSE NULL END)`,
      mssql: `INSERT INTO learning_plan_items (employee_id, course_id, status, progress_percent, completed_at)
              VALUES ($1, $2, $3, $4, CASE WHEN $3 = 'Completed' THEN SYSDATETIMEOFFSET() ELSE NULL END)`,
    }),
...
router.put('/course/:courseId/content/:contentId/progress', async (req, res, next) => {
  try {
    ...
    await query(sql({
      pg: `INSERT INTO employee_content_progress (employee_id, content_item_id) VALUES ($1, $2)
           ON CONFLICT (employee_id, content_item_id) DO NOTHING`,
      mssql: `INSERT INTO employee_content_progress (employee_id, content_item_id)
              SELECT $1, $2 WHERE NOT EXISTS (
                SELECT 1 FROM employee_content_progress WITH (UPDLOCK, HOLDLOCK)
                 WHERE employee_id = $1 AND content_item_id = $2)`,
    }),
